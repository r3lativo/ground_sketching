...
def run_vllm(host, port, model_id, tp_size, gpu_mem_util, seed):
...
if __name__ == "__main__":
    try:
        ...
        enhancer_host = config.get('host')
        enhancer_port = config.get('port')
        enhancer_model = config.get('model_id')
        tp = config.get('tensor_parallel_size')
        mem_util = config.get('gpu_memory_utilization')
        seed = config.get('seed')

        vllm_thread = Thread(target=run_vllm, args=(enhancer_host, enhancer_port, enhancer_model, tp, mem_util, seed), daemon=True)
        vllm_thread.start()
